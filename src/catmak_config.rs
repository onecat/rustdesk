...
fn preset_password_material() -> Option<(&'static str, &'static str)> {
    let storage = option_env!("CATMAK_PRESET_PASSWORD_HASH").unwrap_or("");
    let salt = option_env!("CATMAK_PRESET_PASSWORD_SALT").unwrap_or("");
...
/// Apply CatMak's enforced client policy after any external/custom configuration.
pub(crate) fn apply() {
